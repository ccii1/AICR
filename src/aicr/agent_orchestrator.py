...
class AICRAgent:
    ...
    def run(
        ...
        def _step_rag(ctx: WorkflowContext) -> None:
            ctx.state["rag_answer"] = self.rag.answer(ctx.user_query)

        def _step_kg(ctx: WorkflowContext) -> None:
            graph_facts = self.kg.query("token")
            ctx.state["graph_text"] = "\n".join(
                f"- ({t.subject}) -[{t.predicate}]-> ({t.obj})" for t in graph_facts
            ) or "- 无图谱命中"

        def _step_skill(ctx: WorkflowContext) -> None:
            ctx.state["skill_text"] = self.skills.run("budget_estimator", ctx.user_query)

        def _step_mcp(ctx: WorkflowContext) -> None:
            ctx.state["mcp_text"] = self.mcp.call("policy_checker", ctx.user_query)

        workflow.add_step("rag_retrieval", _step_rag)
        workflow.add_step("graph_lookup", _step_kg)
        workflow.add_step("skill_inference", _step_skill)
        workflow.add_step("policy_check", _step_mcp)
        context = workflow.run(context)

        react = ReActAgent(
            tools={
                "policy_checker": lambda q: self.mcp.call("policy_checker", q),
                ...
                "graph_lookup": lambda q: "\n".join(
                    ...
                    for t in self.kg.query("token")
                )
                or "- 无图谱命中",
            }
        )
        react_trace = react.run(user_query, max_steps=4)
        react_text = "\n".join(
        ...
        review_files_text = ", ".join(review_files) if review_files else "未提供"
