...
class PlanResult:
    ...
    def render(self) -> str:
        ...
        for item in self.results:
            lines.append(f"- {item.agent_name} ({item.focus}): {item.output}")
...
class MultiAgentCoordinator:
    ...
    def _run_plan_a(
        ...
        results = [
            self._exec("compliance-agent", "风险与合规", "policy_checker", prompt_query),
            self._exec("cost-agent", "预算约束", "budget_estimator", prompt_query),
            self._exec("knowledge-agent", "知识检索", "rag_answer", prompt_query),
            self._exec("status-agent", "索引健康", "rag_status", prompt_query),
        ]
        conclusion = "适合正式审批流程，优先确认风险、合规和知识库可追溯性。"
        return PlanResult(
            plan_name="Plan-A",
            strategy="risk-first",
